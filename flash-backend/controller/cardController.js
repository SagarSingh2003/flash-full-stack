...
const cardController = {
    getAllCards : async (req , res) => {
        try{
            ...
            if(deck_id){
                rows = await pool.query(`select * from flashcard WHERE user_id='all' OR deck_id='${deck_id}'`);
            }
    ...
    createCard : async (req ,res) => {
        try {
            ...
            const {question , answer , category , user_id , deck_id} = JSON.parse(req.body.card);
    ...
    createCards : async (req , res) => {
        try {
            ...
            for(let i = 0 ; i < arrOfCardInfo.length ; i++){
                ...
                const {question , answer , category , user_id , deck_id} = arrOfCardInfo[i];
                const [rows] = await pool.query(`INSERT INTO flashcard(question , answer , category , user_id , deck_id) VALUES("${String(question)}" , "${String(answer)}" , "${String(category)}" , "${String(user_id)}" , "${String(deck_id)}");`)
    ...
    deleteAllCards : async(req , res) => {
        try {
            ...
            const [rows] = await pool.query(`DELETE FROM flashcard`)
    ...
    aiGeneratedCards : async(req , res) => {
        try{
            const {user_id ,deck_id, paragraph} = req.body;
